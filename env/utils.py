...
def get_delta_matrix(points, world_size=None, torus=False, add_to_diagonal=0):
    # print(points.shape)
    distance_matrix = [np.array(get_delta_distances(points, p, torus=torus, world_size=world_size)) for p in points]
...
def get_distances(x0, x1, torus=False, world_size=None):
    ...
    # print(dist.shape)
    return dist
def get_delta_distances(x0, x1, torus=False, world_size=None):
    delta = x0 - x1
    if torus:
        delta = np.where(delta > world_size / 2, delta - world_size, delta)
    # dist = np.sqrt((delta ** 2).sum(axis=-1))
