...
class AttentionSAC(object):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.actor_local = SAC(config)
        self.actor_target = SAC(config)
    ...
    def step(self, share_obs,observations):
        ...
        """
        attention_state ,postion_encode= self.attention_net.forward(share_obs,observations)
    ...
    def prep_training(self, device='gpu'):
        ...
        if not self.critic_dev == device:
            ...
            self.critic1_optimizer = Adam(self.critic.critic1.parameters(), lr=self.q_lr,
                                      weight_decay=1e-3)
            self.critic2_optimizer = Adam(self.critic.critic2.parameters(), lr=self.q_lr,
                                      weight_decay=1e-3)
            self.ExpLR_c1 = torch.optim.lr_scheduler.ExponentialLR(self.critic1_optimizer, gamma=0.99)
            self.ExpLR_c2 = torch.optim.lr_scheduler.ExponentialLR(self.critic2_optimizer, gamma=0.99)
            # print("critic")
    ...
    def update_policy(self, sample, **kwargs):
        ...
        for a_i, log_pi, q in zip(range(self.n_agents), log_pi, min_qf_pi):
            policy_loss =((0.2 * log_pi)-q).mean()  # Jπ = 𝔼st∼D,εt∼N[α * logπ(f(εt;st)|st) − Q(st,f(εt;st))]
            total_loss += policy_loss
        

        # self.target_entropy.cuda()
        # self.log_alpha
        self.take_optimisation_step(self.attention_optimizer, [self.actor_local,self.attention_net], total_loss, 0.5)
