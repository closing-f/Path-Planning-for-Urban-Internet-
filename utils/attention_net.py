...
class AttentionNet(nn.Module):
    ...
    def __init__(self, config, action_space=None, norm_in=True, attend_heads=2):
        ...
        self.single_obs_all_dim=config.uav_obs_dim+self.local_obs-self.time_dim #11
        
        self.me_dim_single = (self.n_agents - 1) * self.uav_obs_dim  # 邻居最大维度 x
        self.uav_state_dim = self.uav_obs_dim + self.local_obs  # 508
        self.features_dim_all = self.me_dim_single +self.uav_state_dim
        ...
        for i in range(attend_heads):
            ...
            self.selector_poi.append(nn.Linear(self.single_obs_all_dim, self.attend_poi_dim, bias=False))
    ...
    def forward(self,share_obs,observations):
        
        # input dim is [nr_agent, sum_nei_obs(max_nr, uav_obs)+2+uav_obs] 550
        # print(share_obs.shape) [1, 1024, 400]
        observations = torch.reshape(observations, (self.n_agents, -1, self.features_dim_all)) # nb_uav * obs_dim + localobs
        neibor_input = observations[:, :, :self.me_dim_single] # 6,batchsize,n_agent-1 * obs_dim
        # print(neibor_input.shape) [6, 1024, 40]
        batch_size = neibor_input.shape[1]
        
        # * uav_poi_attentioin
        # print(share_obs.shape)
        uav_self_input = observations[:, :, self.me_dim_single:self.me_dim_single+self.uav_obs_dim+self.local_obs-self.time_dim] # 6,batchsize,5+2
        # print("selector input:")
        # print(uav_self_input)
        uav_poi_input= share_obs
        # print(uav_poi_input) 
        # uav_poi_input= observations[0, :, self.me_dim_single+self.uav_obs_dim+self.local_obs:]# 6,batchsize,400
        uav_poi_input=torch.reshape(uav_poi_input,(batch_size, 100,-1)).permute(1,0,2)
        position_encoding=torch.zeros((self.n_agents, batch_size, self.time_dim),requires_grad=False).to(device=self.device)
        for i in range(self.n_agents):
            position_encoding[i,:]=observations[i, :, self.me_dim_single+self.uav_obs_dim+self.local_obs-self.time_dim:self.me_dim_single+self.uav_obs_dim+self.local_obs]

        # print(uav_self_input.shape) [6, 1024, 18]
        ...
        all_head_poi_selectors = [[sel_ext(enc) for enc in uav_self_input] for sel_ext in self.selector_poi]
        
        # print("selector output:")
        # print(all_head_poi_selectors)
        
        poi_all_values = [[] for _ in range(self.n_agents)]
        poi_all_values=torch.zeros((self.n_agents, batch_size, self.poi_hidden_dim)).to(device=self.device)
        ...
        for head_index, curr_head_keys, curr_head_values, curr_head_selectors in zip(range(self.attend_heads),
            ...
            for i, selector in zip(range(self.n_agents), curr_head_selectors):
                ...
                poi_all_values[i,:,self.attend_poi_dim*head_index:self.attend_poi_dim*(head_index+1)]=other_values
        ...
        # extract selectors for each head for each agent that we're returning Q for
        all_head_neibor_selectors = [[sel_ext(enc) for  enc in (poi_all_values)]
        ...
        for head_index,curr_head_keys, curr_head_values, curr_head_selectors in zip(range(self.attend_heads)
            ...
            for i, key_i,value_i, selector_i in zip(range(self.n_agents), curr_head_keys,curr_head_values, curr_head_selectors):
                ...
                all_attend_logits[i].append(attend_logits)
                all_attend_probs[i].append(attend_weights)
        state_encoding=torch.zeros((self.n_agents, batch_size, self.poi_hidden_dim),requires_grad=True).to(device=self.device)
        ...
        attention_state=neibor_all_values+state_encoding
        # print(attention_state.shape)
        return attention_state,position_encoding
